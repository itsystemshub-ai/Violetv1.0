...
export const useFinanceLogic = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 4. Cargar pagos reportados reales (desde localDb)
      const localReported = await localDb.reported_payments
        .where("tenant_id")
        .equals(tenant.id)
        .toArray();
      setReportedPayments(localReported || []);
    } catch (error) {
  ...
  const handleApproveReportedPayment = async (reportId: string) => {
    try {
      await localDb.reported_payments.delete(reportId);
