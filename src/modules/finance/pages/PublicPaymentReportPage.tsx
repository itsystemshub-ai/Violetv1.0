...
export default function PublicPaymentReportPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      setSubmitted(true);
      toast.success("Pago reportado exitosamente. Será validado en las próximas 24 horas.");
    }, 2000);
  };
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full border-none shadow-2xl rounded-3xl overflow-hidden">
          ...
          <CardContent className="p-8 text-center space-y-4">
            <h2 className="text-2xl font-black text-slate-800">¡Pago Reportado!</h2>
            <p className="text-slate-600">
              Gracias por tu reporte. Hemos recibido la información y nuestro equipo de finanzas la validará pronto.
            </p>
            <Button 
    ...
    <div className="min-h-screen bg-linear-to-br from-indigo-50 to-slate-100 flex flex-col items-center py-12 px-4">
      <div className="w-full max-w-lg space-y-8">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Portal de Pagos Violet</h1>
          <p className="text-slate-500 font-medium">Reporta tu transferencia o depósito en segundos</p>
        </div>

        <Card className="border-none shadow-2xl rounded-3xl overflow-hidden bg-white/80 backdrop-blur-xl">
          <CardHeader className="bg-slate-900 text-white p-8">
            <CardTitle className="text-xl font-bold">Detalles del Reporte</CardTitle>
            <CardDescription className="text-slate-400">Completa todos los campos para agilizar la validación</CardDescription>
          </CardHeader>
          <CardContent className="p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid gap-2">
                <Label className="text-xs font-black uppercase text-slate-500 ml-1">Número de Factura</Label>
                <Input 
                  required 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  <Label className="text-xs font-black uppercase text-slate-500 ml-1">Monto Pagado</Label>
                  <Input 
                    required 
                    type="number" 
                    step="0.01" 
                ...
                <div className="grid gap-2">
                  <Label className="text-xs font-black uppercase text-slate-500 ml-1">Moneda</Label>
              ...
              <div className="grid gap-2">
                <Label className="text-xs font-black uppercase text-slate-500 ml-1">Método de Pago</Label>
                <Select required>
                  <SelectTrigger className="h-12 rounded-xl bg-slate-100/50 border-slate-200 focus:ring-indigo-500">
              ...
              <div className="grid gap-2">
                <Label className="text-xs font-black uppercase text-slate-500 ml-1">Referencia / No. Confirmación</Label>
                <Input 
                  required 
