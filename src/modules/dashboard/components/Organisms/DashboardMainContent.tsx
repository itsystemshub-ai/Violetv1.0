...
import { Landmark, Activity, Archive, Users, Settings2, CheckCircle2, EyeOff, Brain, AlertCircle, Clock } from "lucide-react";
  ...
  if (items && items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center bg-emerald-500/5 rounded-3xl border border-emerald-500/10">
        ...
        <p className="text-sm text-muted-foreground mt-2">No se requieren compras urgentes según la IA.</p>
      </div>
    );
  }
  ...
  return (
    <div className="group relative rounded-3xl p-6 backdrop-blur-xl bg-cyan-500/5 border border-cyan-500/30 transition-all duration-500 h-full hover:shadow-2xl hover:border-cyan-500/60">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-black text-foreground tracking-tight">AI Purchase Suggestions</h3>
          <p className="text-[10px] text-cyan-600 font-black uppercase tracking-widest">Reabastecimiento Inteligente</p>
        </div>
      </div>
      <div className="space-y-3">
        {list.slice(0, 4).map((product, idx) => (
          <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-white/40 dark:bg-slate-900/40 border border-cyan-500/10 hover:bg-white/60 dark:hover:bg-slate-800/60 transition-colors">
            <div className="flex flex-col">
              <span className="text-sm font-bold text-foreground truncate max-w-[140px]">{product.name || product.descripcionManguera}</span>
              <span className="text-[10px] text-muted-foreground uppercase font-black">{product.cauplas || 'S/N'}</span>
            </div>
            <div className="text-right">
              ...
              <div className="text-[10px] text-muted-foreground font-bold">Refill: <span className="text-cyan-600">+{Math.max(product.minStock * 2, 10)}</span></div>
            </div>
          </div>
        ))}
        {list.length > 4 && (
          <div className="pt-2 text-center">
            <Button variant="link" className="text-[10px] h-auto p-0 text-cyan-600 font-black uppercase tracking-widest">
...
const DashboardMainContent: React.FC<DashboardMainContentProps> = ({
  ...
  const { widgetOrder, hiddenWidgets, setWidgetOrder, toggleWidgetVisibility, resetLayout } = useDashboardStore();
        ...
        return (
          <div className="backdrop-blur-xl bg-card/80 border border-border/50 rounded-3xl p-6 h-full shadow-lg">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-black text-foreground tracking-tight">Crecimiento de Ingresos</h3>
    ...
    <div className="col-span-1 lg:col-span-2 space-y-8">
      ...
      <div className="flex items-center justify-between bg-card/50 backdrop-blur-md p-4 rounded-2xl border border-border/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-foreground">Smart Dashboard</h2>
            <p className="text-xs text-muted-foreground font-medium">Personaliza tu vista con drag & drop</p>
          </div>
        </div>
