...
export function InventoryDashboard() {
  ...
  const analysis = useMemo(() => {
    ...
    products.forEach((p: any) => {
      // Rotation
      const salesHistory = p.ventasHistory
        ? Object.values(p.ventasHistory)
        : [];
      let sales = 0;
      for (let i = 0; i < salesHistory.length; i++) {
        sales += Number(salesHistory[i]) || 0;
      }

      if (sales > 50) result.highRotation.push(p);
      else if (sales > 10) result.mediumRotation.push(p);
      else result.lowRotation.push(p);

      // Value
      result.totalValue += (Number(p.stock) || 0) * (Number(p.price) || 0);

      // Stock health
      const stock = Number(p.stock) || 0;
      const minStock = Number(p.minStock) || 0;
      if (stock === 0) result.critical.push(p);
      else if (stock <= minStock) result.warning.push(p);
      else result.healthy.push(p);
    });

    result.avgValue = result.totalValue / (result.total || 1);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-orange-50/30 to-amber-50/20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-page-title bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
          ...
          <div className="text-right">
            <div className="text-numeric-large text-4xl text-orange-600">
              {healthScore}%
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-red-500 to-orange-500 p-6 text-white shadow-2xl">
          ...
          <div className="text-numeric-large text-4xl mb-2">
            {analysis.highRotation.length}
          </div>
          <div className="text-card-title opacity-90">Alta Rotación</div>
          <div className="text-caption opacity-75 mt-1">Productos estrella</div>
          <div className="mt-4 flex items-center gap-2 text-caption">
            <TrendingUp className="w-4 h-4" />
            <span>
              +
              {Math.round(
                (analysis.highRotation.length / analysis.total) * 100,
              )}
              % del catálogo
            </span>
          </div>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-amber-500 to-yellow-500 p-6 text-white shadow-2xl">
          ...
          <div className="text-numeric-large text-4xl mb-2">
            {analysis.mediumRotation.length}
          </div>
          <div className="text-card-title opacity-90">Rotación Media</div>
          <div className="text-caption opacity-75 mt-1">Productos estables</div>
          <div className="mt-4 flex items-center gap-2 text-caption">
            <Minus className="w-4 h-4" />
            <span>
              {Math.round(
                (analysis.mediumRotation.length / analysis.total) * 100,
              )}
              % del catálogo
            </span>
          </div>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-500 to-cyan-500 p-6 text-white shadow-2xl">
          ...
          <div className="text-numeric-large text-4xl mb-2">
            {analysis.lowRotation.length}
          </div>
          <div className="text-card-title opacity-90">Baja Rotación</div>
          <div className="text-caption opacity-75 mt-1">Requieren atención</div>
          <div className="mt-4 flex items-center gap-2 text-caption">
            <TrendingDown className="w-4 h-4" />
            <span>
              {Math.round((analysis.lowRotation.length / analysis.total) * 100)}
              % del catálogo
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="rounded-2xl bg-white dark:bg-slate-900 border-2 border-emerald-200 dark:border-emerald-800 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-numeric-large text-3xl text-emerald-600">
              {analysis.healthy.length}
            </div>
          </div>
          <div className="text-card-title text-emerald-900 dark:text-emerald-100">
            Stock Saludable
          </div>
          ...
          <div className="mt-4 h-2 bg-emerald-100 dark:bg-emerald-900/30 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${(analysis.healthy.length / analysis.total) * 100}%`,
              }}
            />
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-white dark:bg-slate-900 border-2 border-orange-200 dark:border-orange-800 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-numeric-large text-3xl text-orange-600">
              {analysis.warning.length}
            </div>
          </div>
          <div className="text-card-title text-orange-900 dark:text-orange-100">
            Stock Bajo
          </div>
          <div className="text-caption text-muted-foreground mt-1">
          ...
          <div className="mt-4 h-2 bg-orange-100 dark:bg-orange-900/30 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${(analysis.warning.length / analysis.total) * 100}%`,
              }}
            />
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-white dark:bg-slate-900 border-2 border-red-200 dark:border-red-800 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-numeric-large text-3xl text-red-600">
              {analysis.critical.length}
            </div>
          </div>
          <div className="text-card-title text-red-900 dark:text-red-100">
            Sin Stock
          </div>
          <div className="text-caption text-muted-foreground mt-1">
            Acción inmediata
          ...
          <div className="mt-4 h-2 bg-red-100 dark:bg-red-900/30 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${(analysis.critical.length / analysis.total) * 100}%`,
              }}
            />
          </div>
        </div>
      ...
      <div className="rounded-3xl bg-gradient-to-br from-violet-500 via-purple-500 to-fuchsia-500 p-8 text-white shadow-2xl">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-subtitle font-medium opacity-90 mb-2">
              Valor Total del Inventario
            </div>
            <div className="text-numeric-large text-5xl mb-2">
              {formatCurrency(analysis.totalValue)}
            </div>
            <div className="text-subtitle opacity-75">
              Promedio por producto: {formatCurrency(analysis.avgValue)}
            </div>
          </div>
          <div className="text-right">
            ...
            <div className="text-caption opacity-75">Actualizado ahora</div>
          </div>
        </div>
      </div>
    </div>
