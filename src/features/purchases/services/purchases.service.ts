...
export const PurchasesService = {
  ...
  async procesarCompra(data: PurchaseData, tenantId: string) {
    ...
    await AccountingService.postPurchase({
      ...
      tenant_id: tenantId,
      tasa_bcv: data.tasa_bcv // Aseguramos que pase la tasa
    }, 0, 0); // Retenciones se calculan aparte o se pasan aquí
  ...
  async actualizarCostoInventario(productoId: string, nuevaCantidad: number, nuevoPrecioUSD: number) {
    ...
    const costoActualUSD = producto.price || 0; // Asumimos que 'price' en localDb para productos es el costo base

    const stockTotal = stockActual + nuevaCantidad;
    if (stockTotal <= 0) return;

    // CPP = ((StockActual * CostoActual) + (NuevaCant * NuevoCosto)) / StockTotal
    ...
    await SyncService.mutate('products', 'UPDATE', {
      stock: stockTotal,
      price: nuevoCPP, // Actualizamos el costo base del producto
      updated_at: new Date().toISOString()
    }, productoId);
  },

  /**
   * Validador de RIF (Formato Venezolano)
   */
