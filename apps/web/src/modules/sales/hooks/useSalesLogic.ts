...
export const useSalesLogic = () => {
  ...
  const [docType, setDocType] = useState<"venta" | "pedido" | "nota_entrega">("pedido");
  ...
  const handleProcessSale = async () => {
    ...
    if (editingOrderId) {
      try {
        const items = posCart.map(item => {
          const price = item.product.precioFCA || item.product.price;
          return {
    ...
    if (result.success) {
      ...
      if (customerContacto) {
        notifyOrderWhatsApp({
          to: customerContacto,
          orderNumber: controlNumber || result.invoiceId || "PED-" + Date.now(),
          total: cartTotal,
          currency: "USD"
        }).catch(() => {});
  ...
  const cleanDuplicateOrders = async () => {
    try {
      ...
      for (const [_, orders] of Object.entries(ordersByNumber)) {
